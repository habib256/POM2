...
| --- | ---- | ------ | ------ |
| **P0** | Stop growing the god-objects, and **decompose** rather than relocate: the split into `MainWindow_<Area>.cpp` moves rows between files and leaves the coupling intact. Target &lt; 3000 lines/file, POM1 `MainWindow_*` discipline. | 🟢 **the UI half is done 2026-08-23.** `MainWindow.cpp` went 5 590 → 6 622 (the audit that set the target) → 11 511 → **11 154** — the first fall since the rule was written, and it came from removing the SIX parallel per-panel lists (settings load 32, save 32, palette 38, palette dispatch 38, menu rows 37, WASM chrome-light 28), then the 38 `bool showXxx` members, then the ~43 `renderXxxWindow()` calls. One catalog + one registry; adding a panel is a row and a `draw` line. `tools/check_file_sizes.sh` ratchets the ceiling, which now only falls. **Body extraction started 2026-08-23**: the audio panels (mixer, Le Chat Mauve, Mockingboard, Phasor, Echo+) moved verbatim to `MainWindow_AudioPanels.cpp` and the non-audio device panels (Ethernet, SSC, printer, ImageWriter, AI server) to `MainWindow_DevicePanels.cpp` — **11 154 → 9 650 lines** (−1 503), ratchet lowered. **Left**: the storage panels (Disk, Library, SmartPort, FujiNet, FloppyEmu, HDV, cassette) — these reference the `kProDOSHostSentinel` / `freePoNameFor` anonymous-namespace helpers, so the helpers move with them — and the video/misc panels (screen, NTSC, voxel, paint, rewind, keyboard, welcome, memory-viewer, joystick, mouse, no-slot-clock, abstraction). Each is a verbatim move into its own `MainWindow_<Area>.cpp`. | [Arch](#arch-refactor--tooling) `MainWindow.cpp` god-object; [DEV § Panel registry](DEV.md#panel-registry-panelcataloghpanelregistry-mainwindow_panelscpp) |
...
| **P2** | Split `Memory` (`Keyboard` + `PaddleInputs`) **after** an I/O-path test net, not before. The 256-entry `memRead` dispatch is a **perf** job; the split is **compileability**. Do not merge them. | ✅ **DONE 2026-08-23.** Test net first (`input_io_smoke` through `memRead`/`memWrite`), then both extractions: `PaddleInputs` (`PaddleInputs.h` — paddle/button/modifier state + the $C070 RC latch, read at $C061-$C067 and serialised) and `Keyboard` (`Keyboard.h/.cpp` — the latch, the paste FIFO with its control-byte filter/case-fold/cap, the lock-free `$C000` mirror). `Memory` forwards to them and the softswitch path calls `latchMirror()` / `lastKey7()` / `discharging()`. Behaviour-preserving: `input_io_smoke`, `paste_smoke`, `iie_memory_smoke`, both snapshot round-trips and `ui_worker_contention` all green. Memory.cpp 2539 → 2391, hot path unchanged. The `memRead` dispatch (perf) stays a separate open item, as required. | [Memory](#memory-paging--ram-expansion) god-object vs `memRead` hot path — two items |
