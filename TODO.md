...
État 2026-05-27. Backlog dédupliqué, groupé par sévérité (🟠 high ·
...
| 20 | SSI263 speech (chip model)     | AppleWin-faithful| AppleWin `source/SSI263.{h,cpp}` (MAME n'implémente pas)                 | 🟢 formant synth = remplacé par PCM blob 62 phonèmes (AppleWin LGPL → GPL3)                                                               |
...
- [ ] **[Disques] WOZ1 splice point (TRK+6650) ignoré**
  ...
  (`iwm.cpp:218-221`), mais `DiskImage::setWriteSplice` reste un
  stub. Re-master parité Applesauce. **Effort : 1 j.**
...
- [ ] **[SmartPort] ProDOS multi-partition** (feature) — 1 image =
  1 unit = 1 volume aujourd'hui. CFFA3000-style multi-volume non
  supporté. 32 Mo / 16-bit / `.2mg` variantes pinnés.
...
- [ ] **[Printer] Export PDF**. La carte parallèle synthétique
  (`PrinterCard`) est en place : spool host-side + UI « Save as
  .txt » + built-in slot 1 pour //c/+. Reste l'export PDF (renderer
  monospace ou libharu).
- [ ] **[Arch] Config éclatée** : env vars `POM2_*` + CLI flags +
  `Settings`. Centraliser dans un `Config` (env → CLI → Settings →
  defaults) et lister env vars dans `--help`. **Effort : 1 j.**
- [ ] **[Arch] stateMutex partagé CPU+UI**
  (`EmulationController.h:118`). MainWindow_Slots prend ce lock
  pendant plug/unplug — risque jitter audio. Partitionner long terme.
...
- [ ] **[WASM] File picker / drop-zone disks** — bundling
  build-time uniquement. Drop-zone HTML5 →
  `FS.writeFile('/uploads/…')` → `DiskIICard::insert`.
  **Effort : ~1 j.**

## 🟢 Low

- [ ] **[Floppy Emu] modes Dual-5.25" + Smartport-Unit-2** — hors
  scope v1 (4 modes principaux couverts).
...
- [ ] **[UI] `isDuplicate` flagge cffa/smartport35 en double** dans
  la colonne d'assignation Slot Config — cosmétique, le plug reste
  single-instance par défaut.
- [ ] **[Audio] AY Port A read mask par DDR** (R14/R15 academic).
- [ ] **[Audio] Phasor stéréo** — POM2 mixer mono-only ; quand passé
  stéréo, ajouter pan L/R par AY-pair sur Phasor (et SSI263/Echo+).
...
- [ ] **[Features] Le Chat Mauve EVE** (64 KB ext RAM + SPEC1/SPEC2/
...
- [ ] **[Arch] M6502 style hérité** : commentaires FR/EN, casts
  C-style, `void(void)`. `clang-format` + `clang-tidy modernize-*`
  ciblé.
...
- [ ] **[WASM] Audio worklet tuning**. miniaudio backend Web Audio
  fonctionne mais latence par défaut (~150 ms) audible sur speaker
  click ; explorer un `AudioWorkletNode` custom ou réduire le buffer.
...
`§ SmartPortCard`). P1 (CFFA) **fait** ; P2/P3 restent.
...
- [ ] 🟢 **P3 — Apple II SCSI / High-Speed SCSI + CHD**. MAME
  ...
  besoin niche. **Effort : ~30-50 h.** Garder 🟢 sauf demande
  explicite CHD.
...
| ---- | ----------------------------- | ------- | ------------------------------------ |
| 1    | [Disques] UI Force DOS/ProDOS | ~30 min | 🟡 quick win, backend prêt          |
| 2    | [WASM] IDBFS settings persist | 2-4 h   | 🟡 utilisateur web n'a pas de state |
| 3    | [Disques] WOZ1 splice point   | 1 j     | 🟡 Applesauce remaster parity        |
| 4    | [Memory] god-object split     | 2 j     | 🟠 Prépare IIgs + reduces recompile |
