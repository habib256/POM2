...
| **P2** | Split `Memory` (`Keyboard` + `PaddleInputs`) **after** an I/O-path test net, not before. The 256-entry `memRead` dispatch is a **perf** job; the split is **compileability**. Do not merge them. | 🟠 **test net DONE 2026-08-23** (`input_io_smoke`: the $C000/$C010 latch as newest-wins-key + FIFO-paste, the $C061-$C063 buttons + Open/Solid-Apple, the $C064-$C067 paddle RC timing + $C070 re-arm — all through `memRead`/`memWrite`, so the extraction has something to be checked against). The split itself is still open: `Keyboard` (kbMutex + pasteQueue + `kbLatchMirror_` atomic, read on the $C000 hot path) and `PaddleInputs` (paddle/button state + `paddleLatchCycle`, read at $C061-$C067 and serialised in the snapshot) come out of `Memory` behind that net. | [Memory](#memory-paging--ram-expansion) god-object vs `memRead` hot path — two items |
