...
| //c external 3.5" port — own IWM register tracker + bus responder behind `$C0E0-$C0EF`, units from the slot-5 card; the Disk II keeps the 5.25" | `IIcExternalSmartPort.*`, `SmartPortBusPort.h` | [§ //c external port](DEV.md#the-c-external-35-port-iicexternalsmartport) |
...
Current release: **v0.8.5**. **Single source of truth = `CMakeLists.txt`
