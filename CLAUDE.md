...
| Mouse Card (slot 4 by conv.) | `MouseCard.h/.cpp`, `MouseCardAppleWin.h/.cpp` | [DEV.md § Mouse Card](DEV.md#mouse-card) — TWO variants behind the same `setHostMouse` UI plumbing: card key `mouse` = MAME-faithful 68705P3 + 6821 (needs `mouse_341-0270-c.bin` + `mouse_341-0269.bin`), card key `mouseaw` = AppleWin HLE (port of AppleWin `source/MouseInterface.cpp` — only needs the slot EPROM, MCU side is synthesised). Same MC6821 chip for both. Pinned by `mouse_card_smoke` (MAME) + `mouse_card_applewin_smoke` (AppleWin HLE). |
