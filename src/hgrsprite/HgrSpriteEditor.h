...
public:
    ...
    // `aux` is the matching auxiliary-bank snapshot for the DHGR target
    // (nullptr on hosts without one — the DHGR controls are then hidden).
...
private:
    ...
    // ── DHGR target (POM2 addition, gated on host->supportsDhgr()) ──────────
    // The editing canvas stays the mono SHAPE; with the DHGR target on, Stamp/
    // Grab/preview/ASM-export treat each lit shape pixel as one 140-px DHGR
    // colour pixel painted dhgrColor_ (transparent background — only lit
    // pixels stamp). destPxX_ replaces Byte X (pixel granularity).
    bool dhgrTarget_ = false;
    int  dhgrColor_ = 3;                  // 16-colour index for lit pixels
    int  destPxX_ = 20;                   // stamp X in DHGR colour pixels
    const std::vector<uint8_t>* auxMem_ = nullptr;   // per-frame aux snapshot
    // Rasterise the shape (lit → dhgrColor_) into a 16 KB aux+main pair
    // (page-relative) at colour-pixel (px0, 0).
    void buildDhgrPair(std::vector<uint8_t>& pair, int px0) const;
    // Page-relative pair copied from the live snapshots (zeros when absent).
    void snapshotDhgrPair(const std::vector<uint8_t>& memory,
                          std::vector<uint8_t>& pair) const;
    void stampToDhgrPage(const std::vector<uint8_t>& memory);
    void grabFromDhgrPage(const std::vector<uint8_t>& memory);
